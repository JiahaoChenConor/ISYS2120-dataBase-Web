...
def user_in_progress_items(username):
    ...
    try:
        ...
        WHERE progress < 100 AND progress > 0 AND username = %s
...
def get_alltvshows():
    ...
    try:
        ...
        #############################################################################
        sql = """select
                ts.tvshow_id, ts.tvshow_title, count(*) as count
...
def get_song(song_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        select song_id, song_title,  string_agg(artist_name,', ' ORDER BY artist_name) as "artists", length 
...
def get_song_metadata(song_id):
    ...
    try:
        ...
        sql = """
        select * from (select  s.song_id,albummd.md_type_name,albummd.md_value,albummd.md_id
        from mediaserver.song s
        JOIN mediaserver.album_songs als on (s.song_id = als.song_id)
        LEFT OUTER JOIN (mediaserver.albummetadata
                NATURAL JOIN mediaserver.metadata
                NATURAL JOIN mediaserver.metadatatype) albummd
                   on (als.album_id=albummd.album_id)
        union
        select  a.song_id,c.md_type_name,c.md_value,c.md_id
        from  mediaserver.song  a
        left outer join (mediaserver.audiomedia ar natural join mediaserver.MediaItem sa  natural join mediaserver.MediaItemMetadata natural join mediaserver.MetaData natural join mediaserver.MetaDataType) c
                   on (a.song_id=c.media_id)) t1  where t1.song_id = %s
        """
...
def get_podcast(podcast_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
         SELECT p.podcast_id, p.podcast_title, p.podcast_uri, p.podcast_last_updated, 
                comb.md_id, comb.md_value, comb.md_type_name
                FROM mediaserver.podcast p 
                        LEFT OUTER JOIN (mediaserver.podcastmetadata 
                                NATURAL JOIN mediaserver.metadata 
                                NATURAL JOIN mediaserver.metadatatype) comb 
                        ON (p.podcast_id = comb.podcast_id) 
...
def get_podcastep(podcastep_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
                    SELECT pe.media_id, pe.podcast_episode_title, pe.podcast_episode_uri, 
                    pe.podcast_episode_published_date, pe.podcast_episode_length,
                    comb.md_id, comb.md_value, comb.md_type_name
                    FROM  mediaserver.podcastepisode pe LEFT OUTER JOIN (
                                    mediaserver.podcast
                     NATURAL JOIN mediaserver.podcastmetadata 
                     NATURAL JOIN mediaserver.metadata
                     NATURAL JOIN mediaserver.metadatatype) comb ON (pe.podcast_id = comb.podcast_id) 
                    WHERE pe.media_id = %s
        """
...
def get_album(album_id):
    ...
    try:
        ...
        FROM mediaserver.album a
            LEFT OUTER JOIN (mediaserver.albummetadata
                NATURAL JOIN mediaserver.metadata
                NATURAL JOIN mediaserver.metadatatype) albummd USING(album_id)
        WHERE a.album_id = %s
        """
...
def get_album_songs(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT as1.song_id ,s1.song_title,array_to_string(array_agg(distinct ar1.artist_name),',') as artists,s1.length
        FROM mediaserver.album a
            LEFT OUTER JOIN mediaserver.album_songs as1 ON (a.album_id=as1.album_id)
            LEFT OUTER JOIN mediaserver.song s1 ON (as1.song_id=s1.song_id)
            LEFT OUTER JOIN mediaserver.Song_Artists sa1 ON (s1.song_id=sa1.song_id)
            LEFT OUTER JOIN mediaserver.artist ar1 ON (sa1.performing_artist_id=ar1.artist_id)
        WHERE a.album_id = %s
        GROUP BY a.album_id,s1.song_title,as1.track_num,as1.song_id,s1.length
        ORDER BY a.album_id,as1.track_num
        """
...
def get_album_genres(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT distinct songmd.md_id, songmd.md_value as "genres"
                FROM mediaserver.album a
                ...
                WHERE songmd.md_type_name = 'song genre'
                AND album_id = %s
                ORDER BY songmd.md_value;
        """
        ...
        SELECT s.song_id, s.song_title
                FROM mediaserver.song s
                    JOIN (mediaserver.MediaItem
                            NATURAL JOIN mediaserver.MediaItemMetaData
                            NATURAL JOIN mediaserver.metadata
                            NATURAL JOIN mediaserver.metadatatype) songmd ON (s.song_id=songmd.media_id)
                WHERE songmd.md_type_name = 'song genre'
                AND songmd.md_id = %s
                GROUP BY songmd.md_value,songmd.md_id,s.song_id;
        """
...
def get_genre_movies_and_shows(genre_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT distinct tv.tvshow_id as video_id,tv.tvshow_title as video_title,'tvshow' as video_type
        FROM (mediaserver.metadata NATURAL JOIN mediaserver.metadatatype NATURAL JOIN mediaserver.TVShowMetaData) md  NATURAL JOIN mediaserver.TVShow tv
        WHERE md_id=%s
        GROUP BY tv.tvshow_id,tv.tvshow_title
        UNION
        SELECT distinct mov.movie_id,mov.movie_title, 'movie'
        FROM (mediaserver.metadata NATURAL JOIN mediaserver.metadatatype NATURAL JOIN mediaserver.MediaItemMetaData) md JOIN mediaserver.Movie mov ON (mov.movie_id=md.media_id)
        WHERE md_id=%s
        GROUP BY mov.movie_id,mov.movie_title
        ORDER BY video_id;
        """
...
#####################################################
#   Query (10)
#   Get genre type from genre_id CREATED BY PEARL NOT BUILT IN
#####################################################
def get_genre_type(genre_id):
    """
    Get the genre type (md_type_name) from a particular genre_id (md_id) in your media server
    """
    conn = database_connect()
    if(conn is None):
        return None
    ...
    try:
        ...
        sql = """
            SELECT DISTINCT md.md_type_name, md.md_value
            FROM ( mediaserver.metadata
                NATURAL JOIN mediaserver.metadatatype) md
            WHERE md.md_id=%s;
        """
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error getting Genre Data with Genre ID: "+genre_id, sys.exc_info()[0])
        raise
    cur.close()                     # Close the cursor
    conn.close()                    # Close the connection to the db
    return None
...
def get_all_tvshoweps_for_tvshow(tvshow_id):
    ...
    try:
        ...
        #############################################################################
        sql = """select *
        from mediaserver.TVEpisode tve
        where tve.tvshow_id = %s
        order by tve.air_date
        """
...
def find_matchingtvshows(searchterm):
    ...
    try:
        ...
        sql = """
            ...
            order by SIMILARITY(tvshow_title, %s) DESC, tvshow_id limit 20;
            """
...
def find_matchingmovies(searchterm):
    ...
    try:
        ...
        from
            mediaserver.movie m
        order by SIMILARITY(movie_title, %s) DESC, movie_id limit 20;
        """
...
def add_movie_to_db(title,release_year,description,storage_location,genre):
    ...
    try:
        ...
        conn.commit()                   # add the transaction
        r = cur.fetchone()
        print("return val is:")
        print(r)
        cur.close()                     # Close the cursor
...
#   Add a new Song
#####################################################
# title,length, artist, description,storage_location,genre, artwork
def add_song_to_db(title,length, artist, description,storage_location,genre, artwork):
    """
    ...
    try:
        ...
        cur.execute(sql,(storage_location,description,title,length,genre,artist))
...
def multivalue_search(search_item):
    """
    Get all the latest entered movie in your media server
    """
    ...
    try:
        ...
        sql, searchterm = search_item.construct_search()
        r = dictfetchall(cur, sql, searchterm)
        print("return val is:")
        print(r)
        cur.close()                     # Close the cursor
...
def get_allsonggenres():
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """SELECT distinct songmd.md_value as "genre"
                FROM mediaserver.album a
                    LEFT OUTER JOIN mediaserver.album_songs as1 USING (album_id)
                    JOIN (mediaserver.MediaItem
                            NATURAL JOIN mediaserver.MediaItemMetaData
